...
${people.map((p) => `- ${p.name}: ${currency.symbol}${formatCurrency(calculations.perPerson)} ${p.hasPaid ? '✅ Paid' : '⏳ Pending'}`).join('\n')}
...
⏳ Unpaid: ${calculations.unpaidCount} (${currency.symbol}${formatCurrency(calculations.totalUnpaid)})
...
Generated by SuperTool Split Bill Calculator
  ...
  const handleReceiptData = (data: {
    ...
    if (data.tip !== undefined && data.subtotal !== undefined) {
      const tipPct = (data.tip / data.subtotal) * 100
      setTipPercent(String(tipPct.toFixed(1)))
    }

    toast.success('Receipt data applied! Review and adjust if needed.', { duration: 4000 })
  }
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (selected) {
                  ...
                  trackToolEvent('split_bill_currency_change', { currency: selected.code })
                }
              }}
              className={css({
                rounded: 'lg',
